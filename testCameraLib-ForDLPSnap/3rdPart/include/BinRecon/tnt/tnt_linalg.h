...
public:
	...
	TNT::Vector<Real> solve(const TNT::Vector<Real> &B);
...
*/
template <class Real>
TNT::Vector<Real> Cholesky<Real>::solve(const TNT::Vector<Real> &b)
{
	int n = L_.num_rows();
	if (b.dim() != n)
...
class Eigenvalue
{
   ...
   TNT::Vector<Real> d;         /* real part */
   TNT::Vector<Real> e;         /* img part */
...
public:
   ...
   */
   ...
   void getRealEigenvalues (TNT::Vector<Real> &d_) {
      d_ = d;
      return ;
   }
   ...
   */
   void getImagEigenvalues (TNT::Vector<Real> &e_) {
      e_ = e;
      return;
   }
...
class LU
{
   ...
   Matrix<Real> permute_copy( const Matrix<Real> &A, 
   			const TNT::Vector<int> &piv, int j0, int j1)
	{
   ...
   TNT::Vector<Real> permute_copy( const TNT::Vector<Real> &A, 
   		const TNT::Vector<int> &piv)
	{
		int piv_length = piv.dim();
		if (piv_length != A.dim())
			return TNT::Vector<Real>();

		TNT::Vector<Real> x(piv_length);
   ...
   */
   ...
   @param  b   a vector (TNT::Vector> of length equal to the first dimension
   						of A.
   @return x a vector (TNT::Vector> so that L*U*x = b(piv), if B is nonconformant,
   ...
   */
   ...
   TNT::Vector<Real> solve (const TNT::Vector<Real> &b) 
   {
	  ...
	  TNT::Vector<Real> x = permute_copy(b, piv);
...
class SVD 
{
  ...
  public:
   ...
   SVD (const Matrix<Real> &Arg) {
      ...
      n = Arg.num_cols();
      int nu = min(m,n);
      s = TNT::Vector<Real>(min(m+1,n)); 
      U = Matrix<Real>(m, nu, Real(0));
      V = Matrix<Real>(n,n);
      TNT::Vector<Real> e(n);
      TNT::Vector<Real> work(m);
	    Matrix<Real> A(Arg);
      int wantu = 1;  					/* boolean */
      int wantv = 1;  					/* boolean */
	  int i=0, j=0, k=0;
      ...
      for (k = 0; k < max(nct,nrt); k++) {
         if (k < nct) {
            ...
            s[k] = 0;
            for (i = k; i < m; i++) {
                s[k] = TNT::hypot<double>(s[k],A[i][k]);
            }
            if (s[k] != 0.0) {
               if (A[k][k] < 0.0) {
                  s[k] = -s[k];
   ...
   void getSingularValues (TNT::Vector<Real> &x) 
   {
      x = s;
   }
